...
from substrate import render, scopes, stack
...
def _note_text(store, doc_id: str) -> dict:
    """The whole note behind a passage, read from disk.

    Read from the markdown rather than reassembled from chunks, because the note is the artifact
    the user actually owns — but that means the file can have changed since it was indexed, so
    the digest recorded at ingest is compared and `stale` is reported. A note silently newer than
    the passages quoted beside it is exactly the mismatch this project keeps finding.
    """
    import hashlib

    row = store.db.execute(
        "SELECT markdown_path, markdown_sha256 FROM documents WHERE doc_id=?", (doc_id,)
    ).fetchone()
    if row is None:
    ...
    try:
        ...
        raise ToolError(
            ...
            f"above is still valid; the source file has moved or been removed."
        ) from e

    text = raw.decode("utf-8", errors="replace")
    digest = hashlib.sha256(raw).hexdigest()
    body = text[:NOTE_CHAR_CAP]
    return {
        "path": str(path),
        "text": body,
        "n_chars": len(text),
        "truncated": len(text) > NOTE_CHAR_CAP,
        # A file that no longer matches what was indexed: the passages came from the OLD content.
        "stale": bool(row["markdown_sha256"]) and digest != row["markdown_sha256"],
    }
